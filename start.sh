...
original_confs=${NIFI_BASE_DIR}/nifi-tmp/conf
confs_volume=${NIFI_HOME}/conf

# copy conf file if /home/nifi/nifi-current/conf is empty
echo "Check if ${NIFI_BASE_DIR}/nifi-tmp/conf exist"
if test -d ${original_confs}; then
  echo "Check if ${NIFI_HOME}/conf is empty"
    if [ -z "$(ls -A ${confs_volume}/)" ]; then
	    # if no confs is found under ${NIFI_HOME}/conf; copy originals
	    echo "Copying original confs file from temporary location to server..."
	    cp -R ${original_confs}/* ${confs_volume}/
    fi
fi
...
case ${AUTH} in
    ...
    ldap)
        ...
        ;;
    openid)
        echo 'Enabling openidconnect user authentication'
        # Reference openidconnect properties
        ...
        uncomment 'nifi.security.identity.mapping.pattern.dn' ${nifi_props_file}
        uncomment 'nifi.security.identity.mapping.value.dn' ${nifi_props_file}
        prop_replace 'nifi.security.identity.mapping.pattern.dn'  "${NIFI_SEC_ID_MAPPING_PATTERN}"
        prop_replace 'nifi.security.identity.mapping.value.dn'  "${NIFI_SEC_ID_MAPPING_VALUE}"
        if [ ! -z "$OIDC_PROVIDER_TRUSTSTORE_PATH" ]; then
            ...
            echo  "java.arg.oidcProvideTrustStore=-Djavax.net.ssl.trustStore=${OIDC_PROVIDER_TRUSTSTORE_PATH}" >> ${nifi_bootstrap_file}
            echo  "java.arg.oidcProvideTrustStorePwd=-Djavax.net.ssl.trustStorePassword=${OIDC_PROVIDER_TRUSTSTORE_PASSWD}" >> ${nifi_bootstrap_file}
        else
